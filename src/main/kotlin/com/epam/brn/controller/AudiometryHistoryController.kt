...
class AudiometryHistoryController(private val audiometryHistoryService: AudiometryHistoryService) {
    ...
    fun save(@Validated @RequestBody audiometryHistory: AudiometryHistoryRequest): ResponseEntity<BaseResponse<Long>> =
        ResponseEntity.ok().body(BaseResponse(data = audiometryHistoryService.save(audiometryHistory)))
}
