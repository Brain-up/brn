...
@Component
class UploadTaskFromCsvService(
    ...
    @Throws(FileFormatException::class)
    override fun loadTaskFile(file: File): Map<String, String> = uploadTasks(file.inputStream(), null)
    ...
    private fun uploadTasks(inputStream: InputStream, seriesId: Long?): Map<String, String> {
        ...
        if (seriesId != null)
            tasks.forEach { task -> setExerciseSeries(task.value.first, seriesId) }
