...
class CSVParserServiceTest : Spek({
    describe("Csv Parser Service") {
        it("should parse Tasks") {
            ...
            val input = """
                exerciseName orderNumber word audioFileName pictureFileName words wordType
                name1 1 бал no_noise/бал.mp3 pictures/бал.jpg (бам,сам,дам,зал,бак) OBJECT
                name1 3 foo no_noise/foo.mp3 pictures/foo.jpg (foo,bar,baz) OBJECT
                """.trimIndent()
            ...
            result shouldContain TaskCsv(
                "name1", 1, "бал", "no_noise/бал.mp3", "pictures/бал.jpg",
                listOf("(бам", "сам", "дам", "зал", "бак)"), "OBJECT"
            )

            result shouldContain TaskCsv(
                "name1", 3, "foo", "no_noise/foo.mp3", "pictures/foo.jpg",
