...
class AuthorizationAuthenticationIT {
    ...
    internal val pssw: String = "testAdmin"
    ...
    fun initBeforeEachTest() {
        ...
        val password = passwordEncoder.encode(pssw)
    ...
    fun `test get groups with valid credentials`() {
        ...
        val resultAction = this.mockMvc.perform(
            get(BrnPath.GROUPS)
                .with(user(this.email).password(this.pssw).roles("USER", "ADMIN"))
        )
    ...
    fun `test get groups with with no authorities`() {
        ...
        val resultAction = this.mockMvc
            .perform(
                get(BrnPath.GROUPS).with(user(this.email).password(pssw).roles())
            )
    ...
    fun `test login with valid credentials`() {
        // WHEN
        val resultAction = this.mockMvc
            .perform(formLogin().user(this.email).password(this.pssw))
    ...
    fun `test get groups basic authentication`() {
        ...
        val resultAction = this.mockMvc
            .perform(
                get(BrnPath.GROUPS).with(httpBasic(this.email, this.pssw))
            )
