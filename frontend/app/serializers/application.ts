...
type ExerciseDTOType = 'WORDS_SEQUENCES' | 'SENTENCE' | 'SINGLE_SIMPLE_WORDS' | 'PHRASES' | 'DI' | 'DURATION_SIGNALS' | 'FREQUENCY_SIGNALS';
...
export default class ApplicationSerializer extends JSONSerializer {
  ...
  normalizeResponse(store: Store, primaryModelClass: Model, payload: GenericDTO, id: string, requestType: string) {
    const data = payload.data;
    return super.normalizeResponse(store, primaryModelClass, data, id, requestType);
  }

  normalizeSingleResponse(store: Store, primaryModelClass: Model, payload: DTOItem, id: string, requestType: string) {
