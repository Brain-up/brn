...
export default class ProsodyComponent extends Component<ProsodySignature> {
  ...
  @action
  async checkAnswer(selectedLabel: string) {
    this.showTaskResult.perform(selectedLabel);
  }
  ...
  <template>
    ...
    >
      ...
      {{#if this.tasksCopy.length}}
        ...
        <UiTaskContent>
          <ul
            class="prosody__options sm:mx-8 mx-2 mt-2"
            data-test-prosody-options
          >
            {{#each @task.answerOptions key="word" as |answerOption|}}
              <li class="prosody__option">
                <button
                  ...
                  type="button"
                  class="prosody__option-button btn-press rounded transition-colors duration-200
                    {{if
                ...
                >
                  <span class="prosody__label">{{answerOption.word}}</span>
                </button>
              </li>
            {{/each}}
          </ul>
