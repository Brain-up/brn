...
module('Unit | Service | persons', function (hooks) {
  ...
  test('has teamMembers data', function (assert) {
    let service = this.owner.lookup('service:persons');
    assert.ok(service.persons.teamMembers.length === 37);
  });

  test('has teamMembers image', function (assert) {
    let service = this.owner.lookup('service:persons');
    assert.ok(
      service.persons.teamMembers[0].img.includes(
        '/content/pages/description/team-members',
      ),
    );
  });
  ...
  test('has doctors data', function (assert) {
    let service = this.owner.lookup('service:persons');
    assert.ok(
      isArray(service.doctors) &&
        service.doctors.length === 9 &&
        service.doctors[0].img &&
        service.doctors[0].name &&
        service.doctors[0].bio,
    );
  });
  ...
  test('has doctors data in ru language', function (assert) {
    ...
    let service = this.owner.lookup('service:persons');
    assert.ok(service.doctors[0].name === 'Королева Инна Васильевна');
  });
  ...
  test('has doctors data in en language', function (assert) {
    ...
    let service = this.owner.lookup('service:persons');
    assert.ok(service.doctors[0].name === 'Inna Koroleva');
  });
