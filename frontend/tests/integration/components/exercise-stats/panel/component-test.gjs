...
module('Integration | Component | exercise-stats/panel', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
        <template><ExerciseStats::Panel @type="positive" @label="Foo" @value="42"/></template>
    );
    ...
    await render(
        <template><ExerciseStats::Panel @type="total" @label="Foo" @value="42"  @totalStat={{true}}/></template>
    );
  ...
  test('it support bock invocation', async function (assert) {
    await render(
        <template><ExerciseStats::Panel @type="positive" @label="Foo">42</ExerciseStats::Panel></template>
    );
    assert.dom('[data-test-value]').hasText('42');
  });
});
